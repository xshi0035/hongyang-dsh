...
历史设计快照，日期：2026-09-11；对标：`dsh-univer-office` 0.2.14 的包结构与界面标准。下文功能清单与文件树是设计目标，不是完成报告。当前状态见 [Claude 交接文档](CLAUDE_HANDOFF_2026-09-14.zh.md)。
...
## 4. 拟议数据模型（SQLite，`$DSH_HOME/hongyang/finance.db`）
...
工具返回带稳定 ID 的可回放 JSON，客户端 reducer 消费结构化字段，不解析自由文本。当前认领和日报字段由[共享传输类型](../../packages/hongyang/finance/src/shared/wire.ts)定义：
