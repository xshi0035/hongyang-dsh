...
本包由宿主加载且凭据存在时挂载钉钉 Stream 桥接。文字付款先经 provider 预览，明确确认商户后登记。桥接层的图片登记仍待接入。
...
文字补全现使用只读 provider 预览：先发“电费200”，再补商户名称，最后回复“确认 B1-1003”。桥接层按会话和用户保留30分钟草稿，展示数据库候选，只在有效编号确认后写入。取消和过期均不写入，确认失败保留草稿供重试，成功后重复确认不会再次写入。这是有限的文字匹配，尚非通用模型语义理解。草稿暂存在内存中，重启后丢失；原生交互卡片和持久消息去重未完成。图片提取尚未加入草稿流程，桥接层不再自动登记图片。
